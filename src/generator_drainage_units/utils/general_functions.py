...
import pandas as pd
from shapely import Point, LineString, MultiLineString, MultiPoint
...
def split_waterways_by_endpoints(hydroobjects, relevant_culverts):
    ...
    end_points = relevant_culverts['geometry'].apply(lambda line: Point(line.coords[-1]))
    end_points_gdf = relevant_culverts.copy()  # Make a copy to retain all the other data
    ...
    end_points_gdf['geometry'] = end_points_gdf['geometry'].apply(
        lambda geom: Point(geom.x, geom.y) if geom.geom_type == 'Point' and geom.has_z else geom
    )

    # Step 1: Convert MultiLineStrings to LineStrings
    hydroobjects['geometry'] = hydroobjects['geometry'].apply(
        lambda geom: [LineString(part.coords) for part in geom.geoms] if isinstance(geom, MultiLineString) else geom
    )
    ...
    hydroobjects['geometry'] = hydroobjects['geometry'].apply(
        lambda geom: LineString([(x, y) for x, y, *_ in geom.coords]) if geom.has_z else geom
    )
    ...
    combined_points_gdf = gpd.GeoDataFrame(pd.concat([start_points_gdf, end_points_gdf_ho], ignore_index=True), crs="EPSG:28992")
    combined_end_points_gdf = gpd.GeoDataFrame(pd.concat([combined_points_gdf, end_points_gdf], ignore_index=True), crs="EPSG:28992")
    ...
    def split_lines_at_points(gdf_lines, gdf_points):
        ...
        for index, line in gdf_lines.iterrows():
            ...
            for i in range(1, len(coords)):
                segment = LineString(coords[segment_start:i + 1])  # Create segment from start to current point
            ...
            for segment in segments:
                for _, point in gdf_points.iterrows():
                    if segment.distance(point.geometry) < 1e-8:  # Check if the point is very close to the segment
                        # Add point's coordinate twice
                        new_coords = list(segment.coords)
                        new_coords.append((point.geometry.x, point.geometry.y))  # Add point
                        new_coords.append((point.geometry.x, point.geometry.y))  # Add point again
                        ...
                        for j in range(len(new_coords) - 1):
                            new_segment = LineString(new_coords[j:j + 2])  # Create a new segment
                    ...
                    new_lines.append(segment)  # Append the original segment if no point was added
    ...
    def merge_segments(segments_gdf, full_endpoints_gdf):
        ...
        endpoint_coords = set(zip(full_endpoints_gdf.geometry.x, full_endpoints_gdf.geometry.y))
        ...
        for index, row in segments_gdf.iterrows():
            ...
            current_line_coords.extend(list(segment.coords)[1:])  # Avoid duplicating the starting point
            
            # Check if the segment ends at an endpoint from full_endpoints_gdf
            if (segment_end in endpoint_coords):
                current_line_coords.append(segment_end)  # Ensure the endpoint is included
    ...
    def merge_lines_based_on_endpoints(segments_gdf, end_points_gdf):
        ...
        for index, line in segments_gdf.iterrows():
            ...
            endpoint = (current_line.coords[-1][0], current_line.coords[-1][1])  # Get the endpoint as a tuple
            ...
            if endpoint not in endpoint_coords:
                ...
                for next_index, next_line in segments_gdf.iterrows():
                    if line.geometry.coords[-1] == next_line.geometry.coords[0]:  # Check if the start of the next line matches
                        # Merge the two lines
                        current_line = gpd.GeoSeries([current_line, next_line.geometry]).union_all()
    ...
    def remove_covered_lines(merged_gdf):
        ...
        for idx, line in merged_gdf.iterrows():
            ...
            if any(filtered_gdf.geometry.notna() & (filtered_gdf.geometry != line.geometry) & line.geometry.within(filtered_gdf.geometry)):
